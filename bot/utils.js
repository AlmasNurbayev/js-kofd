...
async function uploadToTelegram(chatID, path) {
  ...
  try {
    ...
    await writeError(
      ...
      'bot/utils - uploadToTelegram - read'
    );
  }
  try {
    ...
    await writeError(
      JSON.stringify(err.stack),
      'bot/utils - uploadToTelegram - post'
    );
  }
}
...
async function makeChart(res, chat_id, ctx, mode) {
  ...
  if (res.length > 0) {
    //добавляем имя кассы в каждую строку
    let resAll = [];
    res.forEach((element) => {
      // цикл касс
    ...
    data3.forEach((element) => {
      ...
      if (mode.includes('d')) {
        moment.locale('ru');
        element.date =
          element.date + ' (' + moment(element.date).format('ddd') + ')';
      }
    });
    ...
    chart.setConfig({
      ...
      options: {
        layout: {
          //https://www.chartjs.org/docs/2.9.4/configuration/layout.html
          padding: {
            left: 0,
            right: 50,
            top: 0,
    ...
    try {
      logger.info(
        'bot/utils - sending chart ' + namedir + namefile + ' to ' + chat_id
      );
...
async function ReplyChart(mode, ctx, chat_id) {
  ...
  alarmAdmin(
    ctx,
    'bot/utils - receive /mode/ ' +
      mode +
      ' command by not-admin user: ' +
      ' / ' +
      ctx.from.id +
      ' / ' +
      ctx.from.username
  ...
  if ((await isAdmin(ctx.from.id)) === false) {
    ctx.reply(
      'Вы не входите в разрешенные пользователи, обратитесь к администратору'
    );
    return;
  }
...
async function ReplyData(mode, ctx) {
  ...
  //await ctx.reply('формируются данные по запросу ... ');
  let message =
    'произошла ошибка или сервер не ответил в отведенное время - попробуйте позже';
  console.log('receive request: ' + mode + ' от пользователя ' + ctx.from.id);
  let date = new Date().toLocaleString('ru-RU');
  writeLog(
    `bot_request.txt`,
  ...
  try {
    ...
    message = `Сумма продаж за "${mode}" = ${res.dateStart.slice(
      0,
      10
    )} - ${res.dateEnd.slice(0, 10)}
  Чистое поступление: <b>${res.sumAll.toLocaleString('ru-RU')}</b>`;

    //if (res.sumAll != 0 || res.cashEject != 0) {
    message += `
          кеш: ${res.sumAllCash.toLocaleString(
            'ru-RU'
          )}, карта: ${res.sumAllCard.toLocaleString(
      'ru-RU'
    )}, мобильный: ${res.sumAllMobile.toLocaleString(
      'ru-RU'
    )}, смешано: ${res.sumAllMixed.toLocaleString('ru-RU')}
          В т.ч.:
          Продажи: ${res.sumSale.toLocaleString(
            'ru-RU'
          )}, возвраты: ${res.sumReturn.toLocaleString(
      'ru-RU'
  ...
  Итоги по кассам:`;
    res.obj.forEach((element) => {
      if (
        element.sumAll != 0 ||
        element.cashEject != 0 ||
        element.availableSum != 0
      ) {
        message += `
   - ${element.name_kassa} поступило: <b>${element.sumAll.toLocaleString(
          'ru-RU'
        )}</b>               
      в т.ч. продажи ${element.sumSale.toLocaleString(
        'ru-RU'
      )}, возвраты ${element.sumReturn.toLocaleString(
          'ru-RU'
        )}, выемка ${element.cashEject.toLocaleString('ru-RU')}. `;
        // message += `
  ...
  join "public".kassa on "public".kassa.id_organization  = "public".organization.id`;
    ...
    for await (const element of listKassa) {
      ...
      message += `
    - ${element.name_kassa} - ${Number(
        lastTransaction.availablesum
    ...
    if (mode.includes('день')) {
      bot_message = await ctx.replyWithHTML(
        message,
        Markup.inlineKeyboard([
          Markup.button.callback(
            '🔍все операции',
            'operations-' + res.dateStart.slice(0, 10)
          ),
        ]) //-" + res.dateStart)]),
      );
    } else {
...
function parseResRaws(rows) {
  ...
  rows.forEach((element) => {
    ...
    if (element.type_operation == 6 && element.subtype == 0) {
      elementTypeOper = 'внесение';
      elementSum = element.sum_operation;
    }
...
async function buildMessage(payload) {
  ...
  if (payload) {
    ...
    payload.forEach((element) => {
      const name_kassa = arrKassa.rows.find(
        (e) => e.knumber === element.knumber
      );
    ...
    list.forEach((e, index) => {
      ...
      const dateInButton =
        e.elementFullTime.slice(6, 10) +
        '-' +
        e.elementFullTime.slice(3, 5) +
        '-' +
        e.elementFullTime.slice(0, 2);
      //console.log(dateInButton);
      message += `${index}. ${e.elementKassa} ${
        e.elementTypeOper
      } ${e.elementSum.toLocaleString('ru-RU')} ${e.elementTypePay} ${
        e.elementTime
      }\n ${e.names}\n`;
      buttons.push(
        Markup.button.callback(
